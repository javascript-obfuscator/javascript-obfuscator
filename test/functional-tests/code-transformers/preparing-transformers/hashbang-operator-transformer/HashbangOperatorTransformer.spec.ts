...
describe('HashbangOperatorTransformer', () => {
    ...
    describe('Variant #1: simple', () => {
        const regExp: RegExp = new RegExp(`^#!\/usr\/bin\/env node${lineSeparator}` + `var foo *= *'abc';`);
    ...
    describe('Variant #2: multiple new lines', () => {
        const regExp: RegExp = new RegExp(`^#!\/usr\/bin\/env node${lineSeparator.repeat(4)}` + `var foo *= *'abc';`);
    ...
    describe('Variant #3: `stringArray` option enabled', () => {
        const regExp: RegExp = new RegExp(
            `^#!\/usr\/bin\/env node${lineSeparator}.*` + `${getStringArrayRegExp(['abc']).source}`
        );
    ...
    describe('Variant #4: invalid hashbang indent', () => {
        const regExp: RegExp = new RegExp(`^var foo *= *'abc';`);
    ...
    describe('Variant #5: hashbang as a string value', () => {
        const regExp: RegExp = new RegExp(`^var foo *= *'#!/usr/bin/env\\\\x20node';$`);
